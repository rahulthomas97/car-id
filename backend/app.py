...
# S3 configuration and model download
S3_BUCKET = "segfaultdata"
MODEL_PATH = "models/resnet50.pth"

s3 = boto3.client("s3")
s3.download_file(S3_BUCKET, MODEL_PATH, "/tmp/model_final.pth")
print("Model has been downloaded successfully.", flush=True)

# Load model and define class labels
model = models.resnet50(weights=None)
model.fc = torch.nn.Sequential(
    torch.nn.Dropout(0.2),
    torch.nn.Linear(model.fc.in_features, 196)
)
model.load_state_dict(torch.load("/tmp/model_final.pth", map_location=torch.device("cpu")))
model.eval()
...
@app.get("/ping")
async def ping():
    print("Received ping request", flush=True)
    return {"status": "healthy"}

@app.post("/invocations")
async def invocations(request: Request):
    print("Received POST /invocations request", flush=True)
    ...
    try:
        ...
        print("Error reading image:", e, flush=True)
        raise HTTPException(status_code=400, detail="Invalid image data: " + str(e))
    ...
    try:
        ...
        print("Prediction successful:", predicted_class, flush=True)
        return {"prediction": predicted_class}
    except Exception as e:
        print("Error processing request:", e, flush=True)
        raise HTTPException(status_code=500, detail="Processing failed: " + str(e))

@app.get("/")
async def root():
    print("Received GET / request", flush=True)
    return {"message": "Car ID API is running"}
